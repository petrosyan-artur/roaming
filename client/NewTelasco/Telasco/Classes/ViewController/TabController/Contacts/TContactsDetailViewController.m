...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    NSMutableArray *tempArr = [phones mutableCopy];
    ...
    if (modifidedPhones.count > 0) {
            ...
            for (int i=0; i<arr.count; i++) {
                [rates addObject:[[arr valueForKey:modifidedPhones[i]] valueForKey:@"rate"]];
            }
...
{
    NSLog(@"TContactsDetailViewController call to phone %@", phones[sender.tag]);
    [self callInNumber:modifidedPhones[sender.tag] originPhoneNumber:phones[sender.tag] parent:self];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (rates.count > indexPath.row) {
        if (rates[indexPath.row] != [NSNull null]) {
            ...
            if ([[NSString stringWithFormat:@"%@",rates[indexPath.row]] isEqualToString:kNotFound]) {
                cell.indicator.hidden = YES;
                cell.callBtn.hidden = YES;
                cell.rateLbl.hidden = YES;
            } else {
                cell.rateLbl.text = [NSString stringWithFormat:@"%@", rates[indexPath.row]];
            }
        }
    } else {
