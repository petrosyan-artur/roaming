...
- (void)viewDidLayoutSubviews
{
    ...
    if (!isAnimateState) {
        self.containerView.frame = CGRectMake((self.view.frame.size.width - self.containerView.frame.size.width)/2, (self.view.frame.size.height - self.containerView.frame.size.height)/2, self.containerView.frame.size.width, self.containerView.frame.size.height);
    } else {
        self.containerView.frame = currentContentViewRect;
    }
...
- (void)setTelescoInfo
{
    if (self.appDelegate.telasco.isNewChanges) {
        ...
        self.flagImg.image = [UIImage imageNamed:self.appDelegate.telasco.flagUrl];
        self.phoneTF.text = @"";
    }
}
...
{
    float cvY = [self.view convertPoint:self.containerView.frame.origin toView:self.view].y;
    float cvH = self.containerView.frame.size.height;
    float keyboardY = self.view.frame.size.height - KEYBOARD_HEIGHT;

    if (cvY + cvH > keyboardY) {
        int delta = cvY + cvH - keyboardY;
        CGRect rect = CGRectMake(self.containerView.frame.origin.x, containerViewY-delta, self.containerView.frame.size.width, self.containerView.frame.size.height);
